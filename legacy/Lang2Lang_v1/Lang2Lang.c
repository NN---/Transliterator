...
Lang2LangOptions Options=
{
	...
	closeExit, // enum Close close
	minimize, // enum Minimize minimize
	TRUE, // BOOL bShowTrayIcon
	{ // WindowPos wndPos
		FALSE, // BOOL bMin
		FALSE, // BOOL bMax
		FALSE, // BOOL bHide
	...
	{ // Lang2Lang aLang2Lang[LANG2LANGOPTIONS_LANG2LANG]
		// Eng2Rus
		{
			_T("Eng2Rus"), // TCHAR szName[LANG2LANG_NAME]
		...
		// Rus2Eng
		{
			_T("Rus2Eng"), // TCHAR szName[LANG2LANG_NAME]
			{ // Lang2LangFromTo FromTo[LANG2LANG_FROMTO]
				{_T("a"),{_T("à")}},
				{_T("A"),{_T("À")}},
				{_T("b"),{_T("á")}},
		...
		case WM_INITDIALOG:
			{
				...
				if(Options.bShowTrayIcon)
					NotifyIconAddID(hDlg,TRAYICON_ID,TRAYICON_MSG,g_hIcon,IDS_LANG2LANG);
				// Icon
				SendMessage(hDlg,WM_SETICON,ICON_BIG,(LPARAM)g_hIcon);
				SendMessage(hDlg,WM_SETICON,ICON_SMALL,(LPARAM)g_hIcon);
				// Position
				if(Options.wndPos.rcNormalPosition.bottom&&
					Options.wndPos.rcNormalPosition.top&&
					Options.wndPos.rcNormalPosition.left&&
					Options.wndPos.rcNormalPosition.right)
				{
					if(Options.wndPos.bMin)
					{
						wndpl.showCmd=SW_SHOWMINNOACTIVE;
						if(Options.wndPos.bMax)
					...
					wndpl.rcNormalPosition=Options.wndPos.rcNormalPosition;
				...
				case IDM_OPTIONS:
					{
						...
						if(bOptions)
							if(DisplayOptions(hDlg)==IDOK)
							{
								...
								RegisterHotKey(hDlg,HOTKEY_ID,Options.translationhotkey.fsModifiers,Options.translationhotkey.vk);
								if(!Options.bShowTrayIcon)
									NotifyIconDelete(hDlg,TRAYICON_ID);
								else
									NotifyIconAddID(hDlg,TRAYICON_ID,TRAYICON_MSG,g_hIcon,IDS_LANG2LANG);
							}
					}
				...
				case WM_RBUTTONDOWN:
					{
						...
						GetCursorPos(&pt);
						TrackPopupMenu(hMenu,TPM_LEFTALIGN|TPM_RIGHTBUTTON|TPM_LEFTBUTTON,pt.x,pt.y,0,hDlg,NULL);
		...
		case WM_DESTROY:
			{
				...
				{
					Options.wndPos.bMin=FALSE;
					Options.wndPos.bMax=TRUE;
				}
				else
				if(wndpl.showCmd==SW_SHOWMINIMIZED)
				{
					Options.wndPos.bMin=TRUE;
					Options.wndPos.bMax=wndpl.flags?TRUE:FALSE;
				}
				else // if(wndpl.showCmd==SW_SHOWNORMAL)
				{
					Options.wndPos.bMin=FALSE;
					Options.wndPos.bMax=FALSE;
				}
				Options.wndPos.rcNormalPosition=wndpl.rcNormalPosition;
			}
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
			{
				...
				//remove
				//
				ShowWindow(GetDlgItem(hDlg,IDC_TRANSLATIONTYPING),SW_HIDE);
				//
				//remove
				//
				...
				CheckRadioButton(hDlg,IDC_TRANSLATIONNO,IDC_TRANSLATIONTYPING,IDC_TRANSLATIONNO+Options.translation);
				CheckRadioButton(hDlg,IDC_CLOSEEXIT,IDC_CLOSETOTRAY,IDC_CLOSEEXIT+Options.close);
				CheckRadioButton(hDlg,IDC_MINIMIZE,IDC_MINIMIZETOTRAY,IDC_MINIMIZE+Options.minimize);
		...
		case WM_COMMAND:
			...
			switch(uID)
			{
				...
				case IDC_TRANSLATIONTYPING:
					CheckRadioButton(hDlg,IDC_TRANSLATIONNO,IDC_TRANSLATIONTYPING,uID);
				...
				case IDC_MINIMIZETOTRAY:
					CheckRadioButton(hDlg,IDC_MINIMIZE,IDC_MINIMIZETOTRAY,uID);
					break;
			}
			break;
		case WM_NOTIFY:
				...
				case PSN_APPLY:
					{
						...
						if(IsDlgButtonChecked(hDlg,IDC_MINIMIZE))
		...
		case IDC_LANG2LANG:
			switch(HIWORD(wParam))
			{
				case CBN_SELCHANGE:
				{
					...
					if(pLang2Lang!=Options.aLang2Lang+iCurSel)
					{
						Lang2LangFromTo *pLang2LangFromTo;
						int i;
						pLang2Lang=Options.aLang2Lang+iCurSel;
