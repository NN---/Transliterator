...
void CTranslationDlg::UpdateTranslation()
{
	...
	NN::auto_array<char> aText(nSize);
	if(!aText.data())
		return;
	MakeTranslation(aText.data());

	NN::auto_array<wchar_t> awText(nSize);
	if(!awText.data())
		return;
	::MultiByteToWideChar(1251,0,aText.data(),nSize,awText.data(),nSize);

	m_reTranslation.SetTextEx((LPTSTR)awText.data(),ST_DEFAULT,1200);
