...
void CTransliteratorDlg::Translate(Translate::Direction dir)
{
	...
	NN::AutoArray<wchar_t> aText(nChar*2);
	if(!aText.GetData())
		return;
	
	{
		NN::AutoSGRef<BOOL> sg(g_Options.m_bTranslate,FALSE,g_Options.m_bTranslate);

		reFrom.GetTextEx((LPSTR)aText.GetData(),nChar*sizeof(wchar_t),GT_DEFAULT,1200);
		g_Language.Translate(aText.GetData(),dir,nChar);
		reTo.SetTextEx((LPCSTR)aText.GetData(),ST_DEFAULT,1200);
	}
}
...
void CTransliteratorDlg::LoadSettings()
{
	...
	if(!regProg.Open(HKEY_CURRENT_USER,"Software\\NN\\Transliterator"))
	{
		...
		for(int i=0;i<2;i++)
		{
			m_aText[i].Allocate(nChar);
			if(m_aText[i].GetData())
				if(LoadDataSize(regProg,astrTrnsRusText[i],m_aText[i].GetData(),nChar))
					m_aText[i].Free();
		}
	}
	else
...
void CTransliteratorDlg::SaveSettings()
{
	...
	for(int i=0;i<2;i++)
	{
		...
		m_aText[i].Allocate(anChar[i]);
		if(!m_aText[i].GetData())
			break;
	
		m_reTrnsRus[i].GetTextEx((LPSTR)m_aText[i].GetData(),anChar[i]*sizeof(wchar_t),GT_DEFAULT,1200);

		regProg.SetBinaryValue("TrnsText",m_aText[Trns].GetData(),anChar[i]*sizeof(wchar_t));
		regProg.SetBinaryValue("RusText",m_aText[Rus].GetData(),anChar[i]*sizeof(wchar_t));
		m_aText[i].Free();
	}
	m_aText[Trns].Free();
	m_aText[Rus].Free();
...
void CTransliteratorDlg::HotKeyText(Translate::Direction dir,HWND hWndTransliterator,void (*pDo)(char*,Translate::Direction,int,HWND))
{
		...
		_tcsstr(szClassEdit, "TEXT")))
	{
		...
		{
			wndTranslation.SendMessage(WM_GETTEXT,nChar-1,(LPARAM)aText.GetData());
			
			(*pDo)(aText.GetData(),dir,nChar,hWndTransliterator);
						
			wndTranslation.SendMessage(WM_SETTEXT,0,(LPARAM)aText.GetData());
		}
		else
		{
			wndTranslation.SendMessage(WM_GETTEXT,nChar-1,(LPARAM)aText.GetData());
			...
			(*pDo)(aText.GetData(),dir,nChar-(nEnd-nStart),hWndTransliterator);
		...
		if(LPFNOBJECTFROMLRESULT pfObjectFromLresult = (LPFNOBJECTFROMLRESULT)::GetProcAddress(hOleAccInstance, _T("ObjectFromLresult")))
		{
			if(SUCCEEDED((*pfObjectFromLresult)(lRes, IID_IHTMLDocument, 0, (void**)&spDoc)))
			{
				...
				if(strText)
				{
					...
					{
						NN::AutoArray<char> aTextA(nChar);
						if(!aTextA.GetData())
							return;
						::WideCharToMultiByte(CP,0,strText.m_str,nChar,aTextA.GetData(),nChar,0,0);
						(*pDo)(aTextA.GetData(),dir,nChar,hWndTransliterator);
						strText.Empty();
						strText.Append(aTextA.GetData());
					}
	...
	else
	{
		...
		NN::AutoArray<TCHAR> aText(nChar);
		if(!aText.GetData())
			return;

		::SendMessage(hWndTranslation,WM_GETTEXT,(WPARAM)(nChar-1),(LPARAM)aText.GetData());

		(*pDo)(aText.GetData(),dir,nChar,hWndTransliterator);

		::SendMessage(hWndTranslation,WM_SETTEXT,0,(LPARAM)aText.GetData());
	}
}
...
void CTransliteratorDlg::InitControls()
{
	...
	if(bCreated)
	{
		for(int i=0;i<2;i++)
		{
			...
			aText[i].Allocate(nChar);
			if(aText[i].GetData())
				m_reTrnsRus[i].GetTextEx((LPSTR)aText[i].GetData(),nChar*sizeof(wchar_t),GT_DEFAULT,1200);
			// Destroy
	...
	if(bCreated)
	{
		for(int i=0;i<2;i++)
			if(aText[i].GetData())
				m_reTrnsRus[i].SetTextEx((LPCSTR)aText[i].GetData(),ST_DEFAULT,1200);
	}
...
LRESULT CTransliteratorDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	{
		NN::AutoSGRef<BOOL> sg(g_Options.m_bShowTrayIcon,FALSE,g_Options.m_bShowTrayIcon);
		SetOptions(true);
	}
	...
	// Set Text
	if(m_aText[Trns].GetData()||m_aText[Rus].GetData())
	{
		NN::AutoSGRef<BOOL> sg(g_Options.m_bTranslate,FALSE,g_Options.m_bTranslate);
		for(int i=0;i<2;i++)
		{
			if(m_aText[i].GetData())
			{
				m_reTrnsRus[i].SetTextEx((LPCSTR)m_aText[i].GetData(),ST_DEFAULT,1200);
				m_aText[i].Free();
			}
		}
	}
...
LRESULT CTransliteratorDlg::OnEditIgnore(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	NN::AutoArray<wchar_t> aText(nChar);
	long nStart,nEnd;
	re.GetSel(nStart,nEnd);

	re.GetTextEx((LPSTR)aText.GetData(),nChar*sizeof(wchar_t),GT_DEFAULT,1200);
	// Insert/Remove
	if(nStart==nEnd)
	{
		if(bInsert)
		{
			g_Language.SetIgnore(aText.GetData(),nChar-1,re==m_reTrnsRus[Trns],bInsert);
		
			re.SetTextEx((LPCSTR)aText.GetData(),ST_DEFAULT,1200);

			int nText=lstrlenW(aText.GetData());
	...
	{
		if(!nStart&&nEnd==-1||
			!nStart&&nEnd==lstrlenW(aText.GetData())) // All
		{
			g_Language.Ignore(aText.GetData(),nChar-1,re==m_reTrnsRus[Trns],bInsert);
			re.SetTextEx((LPCSTR)aText.GetData(),ST_DEFAULT,1200);
		}
		else // Selection
		{
			...
			g_Language.Ignore(aText.GetData(),nChar-1,re==m_reTrnsRus[Trns],bInsert);
			NN::AutoArray<char> aTextA(nChar);
			::WideCharToMultiByte(CP,0,aText.GetData(),nChar,aTextA.GetData(),nChar,0,0);
			re.ReplaceSel(aTextA.GetData());
		}
	}
...
{
	NN::AutoSGRef<BOOL> sg(g_Options.m_bTranslate,FALSE);
	...
	NN::AutoArray<wchar_t> aText(nLen);
	reFrom.GetTextEx((LPSTR)aText.GetData(),nLen,GT_DEFAULT,CP);
	reTo.SetTextEx((LPCSTR)aText.GetData(),ST_DEFAULT,CP);
